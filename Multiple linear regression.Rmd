...
This one: Yes, revise wording to "Is there an association between beauty and course evaluations?"
</ol>
...
This one because is actually the opposite: The left skewness of the data suggests that the students are less likely to rate the professors highly.
</ol>
```{r score-dist}
...
Here we have the relation between age and gender. This shows that in general female professors are younger than male profesors.
</div>
```{r two-vars-eda}
...
The problem wit geom_point() is that this data has little amount of unique values  creating an overplotting problem.
</div>
...
Answer: True but R squared is very low!
</ol>
...
This is wrong since this interpretation implies an histogram or a normal probability plot: Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. 
</ol>
...
The order would be: Tenure Track, Tenured, Teaching
</ol>
...
If the research question is correct we should expect bty_avg to be the most relevant variable. But we can also expect the age variable to play an important role, having the younger professors a higher score and bty_avg.
</div>
...
If we take out rank, then:

score ~ ethnicity + gender + language + age + cls_perc_eval + cls_students + cls_level 
       + cls_profs + cls_credits + bty_avg + pic_outfit + pic_color
...
The answer :0.12 points higher than minority professors, all else held constant
</ol>
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full2 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval + cls_students + cls_level + cls_profs + cls_credits + bty_avg + pic_outfit + pic_color, data = evals)
summary(m_full2)
summary(m_full)
```
<ol>
...
```{r eval=FALSE}
summary(m_full2)$adj.r.squared
```

Then, try dropping the next variable from the full model (`ethnicity`):

```{r eval=FALSE}
m_full3 = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full3)$adj.r.squared
```
...
# type your code for the Exercise here, and Knit
#Without gender
m_full4 = lm(score ~ rank +ethnicity + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full4)$adj.r.squared

#Without language
m_full5 = lm(score ~ rank +ethnicity + gender + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full5)$adj.r.squared

#Without age
m_full6 = lm(score ~ rank +ethnicity + gender + language + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full6)$adj.r.squared

#Without cls_perc_eval
m_full7 = lm(score ~ rank +ethnicity + gender + language + age + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full7)$adj.r.squared

#Without cls_students
m_full8 = lm(score ~ rank +ethnicity + gender + language + age + 
     cls_perc_eval + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_full8)$adj.r.squared
...
The best model is the one that eliminates rank $R^2= 0.1587245$
</ol>
...
Yep, there is no independence in the data! Which violates the independence of outcomes
</div>
...
No, since this data only applies to professors of the university of Texas of such and such characteristics (USA, Texas, x1....xn class, etc.)
</div>
