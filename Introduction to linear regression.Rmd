...
Probably since looks linear, but also it is important to notice that the strenght relation seems small
</div>
...
Answer is : The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats.
</ol>
...
The answer is : For each additional home run, the model predicts 1.83 more runs, on average
</ol>
...
This one is false: The residuals show a curved pattern
</ol>
...
This is the answer: The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met
</ol>
...
plot_ss(data = mlb11, x=homeruns, y=runs, showSquares = TRUE)
...
Answer is= the best predictor is batting average since R squared =0.6438. Therefore it explains 64% of observed variance!
</ol>
```{r runs-vs-others}
...
Answer would be that non of the options have all of the conditions, but if we ignore that new_obs has a higher R squared value
</ol>
```{r runs-vs-new-vars}
# type your code for Question 10 here, and Knit
m_newobs= lm(runs~new_obs, data= mlb11)
m_newlug= lm(runs~new_slug, data= mlb11)
m_newonbase= lm(runs~new_onbase, data= mlb11)

#Assumptions of new_obs
#Linearity
...
#Homoscedasticity
ggplot(data=m_newobs, aes(x=.fitted, y=.resid))+
  geom_point()+
  geom_hline(yintercept = 0, linetype= "dashed")+
  xlab("Fitted")+
  ylab("Residuals")
...
ggplot(data=m_newobs,aes(x=.resid))+
  geom_histogram(fill="light blue", color= "black")

#Assumptions of new_lug
ggplot(data= mlb11, aes(x =new_slug, y=runs))+
  geom_point()+
  stat_smooth(method = "lm", se= F)

ggplot(data=m_newlug, aes(x=.fitted, y=.resid))+
  geom_point()+
  geom_hline(yintercept = 0, linetype= "dashed")+
  xlab("Fitted")+
  ylab("Residuals")

ggplot(data=m_newlug,aes(x=.resid))+
  geom_histogram(fill="light blue", color= "black")

#Assumptions new_onbase
ggplot(data= mlb11, aes(x =new_onbase, y=runs))+
  geom_point()+
  stat_smooth(method = "lm", se= F)

ggplot(data=m_newonbase, aes(x=.fitted, y=.resid))+
  geom_point()+
  geom_hline(yintercept = 0, linetype= "dashed")+
  xlab("Fitted")+
  ylab("Residuals")

ggplot(data=m_newonbase,aes(x=.resid))+
  geom_histogram(fill="light blue", color= "black")

#If we want the linear regresion data, then we can use
summary(m_newlug)
summary(m_newobs)
summary(m_newonbase)
```
